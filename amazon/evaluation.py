...
class DataHandler:
    def __init__(self):
        ...
        self.uu_file = predir + "uu10_csr"
        self.ii_file = predir + "ii10_csr"
    ...
    def make_cuda_tensor(self, csr_mat):
        mat = csr_mat.tocoo()  # 先转成 COO 格式
    ...
    def LoadData(self):
        ...
        with open(self.uu_file, "rb") as f:
            uu_csr = pickle.load(f)

        with open(self.ii_file, "rb") as f:
            ii_csr = pickle.load(f)

        self.uu_csr_tensor = self.make_cuda_tensor(uu_csr)
        self.ii_csr_tensor = self.make_cuda_tensor(ii_csr)
...
def batch_evaluation(trained_model, csr_test, csr_train, epoch, test_batch, topk, max_K, device='cuda:0'):
